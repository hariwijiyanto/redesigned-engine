...
// FUNGSI BLOOM FILTER - Tetap digunakan untuk filtering awal
...
// KERNEL UNTUK GENERASI TRAP TABLE DENGAN BIG INTEGER
extern "C"
__global__ void generate_trap_table_kernel_bigint(
    ...
    // Hitung private key untuk thread ini: startK + idx
    BigInt privkey_bi;
    BigInt idx_bigint;
    init_bigint(&idx_bigint, idx);
    ptx_u256Add(&privkey_bi, &const_startK, &idx_bigint);

    // Hitung titik publik: G × privkey
    ECPointJac point_jac;
    scalar_multiply_jac_precomputed(&point_jac, &privkey_bi);
    ...
    // Simpan ke trap table - HANYA SIMPAN INDEX
    d_trap_table[idx].fp = fp;
    d_trap_table[idx].index = idx;  // Simpan index saja, full key = startK + idx
...
// KERNEL UNTUK SEARCH - Tetap digunakan untuk filtering cepat
extern "C"
__global__ void search_kernel(
    TrapEntry* d_trap_table,
    uint32_t* d_bloom_filter,
    uint32_t trap_table_size,
    uint64_t bloom_size,
    uint64_t target_fp,
    unsigned long long int* d_found_results
) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        // Check bloom filter first
        if (!check_bloom_filter(d_bloom_filter, bloom_size, target_fp)) {
            return;
        }

        // Linear search untuk memastikan tidak ada masalah binary search
        for (int i = 0; i < trap_table_size; i++) {
            if (d_trap_table[i].fp == target_fp) {
                // Found match
                d_found_results[0] = d_trap_table[i].index;  // Ini adalah index
                d_found_results[1] = 1;  // Flag found
                return;
            }
        }
    }
...
// KERNEL BATCH UNTUK MULTIPLE TARGETS
extern "C"
__global__ void batch_search_kernel(
    ...
    uint32_t trap_table_size,
    uint64_t bloom_size,
    uint64_t* d_target_fps,          // Array of target fingerprints
    uint32_t batch_size,             // Number of targets in this batch
    unsigned long long int* d_found_results  // [index_found, offset_index, found_flag]
) {
    int target_idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (target_idx >= batch_size) return;

    uint64_t target_fp = d_target_fps[target_idx];
    ...
    // Linear search in trap table
    for (int i = 0; i < trap_table_size; i++) {
        if (d_trap_table[i].fp == target_fp) {
            // Found match - store results
            d_found_results[target_idx * 3 + 0] = d_trap_table[i].index;
            d_found_results[target_idx * 3 + 1] = target_idx;  // Which offset this corresponds to
            d_found_results[target_idx * 3 + 2] = 1;  // Found flag
            return;
        }
    }
}
